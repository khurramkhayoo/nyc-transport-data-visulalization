...
---
📊 Datasets
Three publicly available datasets from the MTA Open Data Portal were used:
...
Daily Ridership by Transit Mode — Stacked area chart showing Subway, Bus, LIRR, MNR, and SIR usage over time
Weekday vs Weekend Comparison — Grouped bar chart comparing average ridership across transit modes
Ridership Heatmap — Hour × Day-of-Week heatmap revealing peak commute patterns
Hourly Ridership Curve — Weekday vs weekend hourly ridership profile (dual-peak M-shape vs single hump)
Top 20 Busiest Stations — Horizontal bar chart of highest-traffic subway stations
Borough-Level Analysis — Pie charts showing ridership distribution and per-station averages by borough
Interactive Subway Map — Folium-based geographic heatmap of station-level ridership across NYC
Payment Method Breakdown — Pie chart showing OMNY vs MetroCard adoption
COVID-19 Recovery Timeline — Monthly average subway ridership from 2020 to 2026
Day-of-Week Ridership — Bar chart of average subway ridership by day of the week
---
🔍 Key Insights
...
```bash
   ...
   ```
Place the dataset CSV files in the `data/` folder
